...
const STORAGE_KEY = 'snowfield-trigger-configs';
...
export default function SnowfieldTriggerDebug() {
  ...
  const handleExportCode = () => {
    ...
    if (triggerMarkers.length > 0) {
      ...
      triggerMarkers.forEach((trigger, i) => {
        ...
        code += `    targetUrl: "/stage/snowfield",\n`;
  ...
  const handleExportAll = () => {
    ...
    let code = `// Snowfield stage configurations - Generated from debug tool\n`;
    code += `// Configured maps: ${configuredMaps.length}\n\n`;
    code += `export const SNOWFIELD_CONFIG: Record<string, MapConfig> = {\n`;
