...
export default function TowerArea({ mapId, mapName, spawnPosition: defaultSpawn = [0, 10, 0], spawnRotation: defaultRotation = 0, triggers = [], timeOfDay = 'day', weather = 'clear', children }: TowerAreaProps) {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: 'linear-gradient(135deg, #1a1510 0%, #2a2520 100%)', color: 'white' }}>
        <p>Loading {mapName}...</p>
      </div>
    );
  }
  ...
  if (!selectedCharacter) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: 'linear-gradient(135deg, #1a1510 0%, #2a2520 100%)', color: 'white', flexDirection: 'column', gap: '1rem' }}>
        <p>No character selected</p>
        <a href="/character-select" style={{ padding: '0.75rem 2rem', background: 'white', color: '#1a1510', textDecoration: 'none', borderRadius: '8px', fontWeight: 'bold' }}>
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 1000, background: 'rgba(0,0,0,0.7)', color: 'white', padding: '10px', borderRadius: '5px', fontFamily: 'monospace', fontSize: '12px' }}>
        ...
        <div style={{ marginTop: '0.5rem', color: '#d4b896' }}>Press SPACE: {debugStart ? 'Debug Start' : 'Debug Stop'}</div>
      </div>
      ...
      <Canvas shadows>
        ...
        <Suspense fallback={null}>
          ...
          <EternalMoteParticles intensity="normal" />
