...
function ParuDebugScene({
  ...
  useEffect(() => {
    ...
    const paruDir = getParuDir(selectedMap);
    const glbPath = `/${paruDir}/${selectedMap}/lndmd/${selectedMap}_m.glb`;
    ...
    loader.load(glbPath, (gltf) => {
      ...
      const processTexture = (
        ...
        textureList.push({ name: displayName, key, filename, texture, material });
      };
...
const PARU_A_MAPS = [
  's05a_ga1', 's05a_ib1', 's05a_ib2', 's05a_ic1', 's05a_ic3',
  's05a_lb1', 's05a_lb3', 's05a_lc1', 's05a_lc2', 's05a_na1',
  's05a_nb2', 's05a_nc2', 's05a_sa1', 's05a_tb3', 's05a_tc3',
  's05a_td1', 's05a_td2', 's05a_xb2'
];
const PARU_B_MAPS = [
  's05b_ga1', 's05b_ib1', 's05b_ib2', 's05b_ic1', 's05b_ic3',
  's05b_lb1', 's05b_lb3', 's05b_lc1', 's05b_lc2', 's05b_na1',
  's05b_nb2', 's05b_nc2', 's05b_sa1', 's05b_tb3', 's05b_tc3',
  's05b_td1', 's05b_td2', 's05b_xb2'
];
const PARU_E_MAPS = ['s05e_ia1'];
const PARU_Z_MAPS = ['s05z_na1'];

export default function ParuDebug() {
  const [selectedMap, setSelectedMap] = useState('s05a_ga1');
  ...
  const handleExportJSON = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'paru_texture_config.json'; a.click();
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex' }}>
      <div style={{ width: '320px', height: '100vh', background: '#1a1a1a', color: 'white', padding: '15px', fontFamily: 'monospace', fontSize: '12px', overflowY: 'auto', boxSizing: 'border-box', flexShrink: 0 }}>
        <div style={{ fontWeight: 'bold', marginBottom: '5px', fontSize: '14px' }}>Oblivion City Paru - Texture Debug</div>
        ...
        <div style={{ marginBottom: '15px' }}>
          ...
          <select value={selectedMap} onChange={(e) => setSelectedMap(e.target.value)} style={{ width: '100%', padding: '8px', background: '#333', color: 'white', border: '1px solid #555', borderRadius: '4px', fontFamily: 'monospace', fontSize: '12px', cursor: 'pointer' }}>
            <optgroup label="Paru A">{PARU_A_MAPS.map((map) => (<option key={map} value={map}>{map}</option>))}</optgroup>
            <optgroup label="Paru B">{PARU_B_MAPS.map((map) => (<option key={map} value={map}>{map}</option>))}</optgroup>
            <optgroup label="Paru E">{PARU_E_MAPS.map((map) => (<option key={map} value={map}>{map}</option>))}</optgroup>
            <optgroup label="Paru Z">{PARU_Z_MAPS.map((map) => (<option key={map} value={map}>{map}</option>))}</optgroup>
          </select>
        </div>
        {textures.length > 0 && (<>
          <div style={{ marginBottom: '15px' }}>
        ...
        <a href="/stage/paru" style={{ display: 'block', width: '100%', padding: '10px', background: '#c83232', color: 'white', textDecoration: 'none', borderRadius: '4px', fontFamily: 'monospace', fontSize: '12px', fontWeight: 'bold', textAlign: 'center', boxSizing: 'border-box' }}>Back to Paru</a>
      </div>
      <div style={{ flex: 1, height: '100vh' }}>
        <Canvas shadows>
          ...
          <Suspense fallback={null}><ParuDebugScene selectedMap={selectedMap} textureSettings={textureAdjustments} onTexturesLoaded={handleTexturesLoaded} /></Suspense>
        </Canvas>
      </div>
    </div>
  );
