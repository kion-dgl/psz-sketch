...
function ArcaDebugScene({
  ...
  useEffect(() => {
    ...
    const arcaDir = getArcaDir(selectedMap);
    const glbPath = `/${arcaDir}/${selectedMap}/lndmd/${selectedMap}_m.glb`;
...
const ARCA_A_MAPS = ['s06a_ga1', 's06a_ib1', 's06a_ib2', 's06a_ic1', 's06a_ic3', 's06a_lb1', 's06a_lb3', 's06a_lc1', 's06a_lc2', 's06a_na1', 's06a_nb2', 's06a_nc2', 's06a_sa1', 's06a_tb3', 's06a_tc3', 's06a_td1', 's06a_td2', 's06a_xb2'];
const ARCA_B_MAPS = ['s06b_ga1', 's06b_ib1', 's06b_ib2', 's06b_ic1', 's06b_ic3', 's06b_lb1', 's06b_lb3', 's06b_lc1', 's06b_lc2', 's06b_na1', 's06b_nb2', 's06b_nc2', 's06b_sa1', 's06b_tb3', 's06b_tc3', 's06b_td1', 's06b_td2', 's06b_xb2'];
const ARCA_E_MAPS = ['s06e_ia1'];
const ARCA_Z_MAPS = ['s06z_na1'];

export default function ArcaDebug() {
  const [selectedMap, setSelectedMap] = useState('s06a_ga1');
  ...
  const handleExportJSON = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'arca_texture_config.json'; a.click();
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex' }}>
      <div style={{ width: '320px', height: '100vh', background: '#1a1a1a', color: 'white', padding: '15px', fontFamily: 'monospace', fontSize: '12px', overflowY: 'auto', boxSizing: 'border-box', flexShrink: 0 }}>
        <div style={{ fontWeight: 'bold', marginBottom: '5px', fontSize: '14px' }}>Arca Plant - Texture Debug</div>
        ...
        <div style={{ marginBottom: '15px' }}>
          ...
          <select value={selectedMap} onChange={(e) => setSelectedMap(e.target.value)} style={{ width: '100%', padding: '8px', background: '#333', color: 'white', border: '1px solid #555', borderRadius: '4px', fontFamily: 'monospace', fontSize: '12px', cursor: 'pointer' }}>
            <optgroup label="Arca A">{ARCA_A_MAPS.map((map) => (<option key={map} value={map}>{map}</option>))}</optgroup>
            <optgroup label="Arca B">{ARCA_B_MAPS.map((map) => (<option key={map} value={map}>{map}</option>))}</optgroup>
            <optgroup label="Arca E">{ARCA_E_MAPS.map((map) => (<option key={map} value={map}>{map}</option>))}</optgroup>
            <optgroup label="Arca Z">{ARCA_Z_MAPS.map((map) => (<option key={map} value={map}>{map}</option>))}</optgroup>
          </select>
        </div>
        {textures.length > 0 && (<>
          <div style={{ marginBottom: '15px' }}>
        ...
        <a href="/stage/arca" style={{ display: 'block', width: '100%', padding: '10px', background: '#c83232', color: 'white', textDecoration: 'none', borderRadius: '4px', fontFamily: 'monospace', fontSize: '12px', fontWeight: 'bold', textAlign: 'center', boxSizing: 'border-box' }}>Back to Arca</a>
      </div>
      <div style={{ flex: 1, height: '100vh' }}>
        <Canvas shadows>
          ...
          <Suspense fallback={null}><ArcaDebugScene selectedMap={selectedMap} textureSettings={textureAdjustments} onTexturesLoaded={handleTexturesLoaded} /></Suspense>
        </Canvas>
      </div>
    </div>
  );
