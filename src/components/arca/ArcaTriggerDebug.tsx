...
const STORAGE_KEY = 'arca-trigger-configs';
...
const ALL_MAPS = [...ARCA_A_MAPS, ...ARCA_B_MAPS, ...ARCA_E_MAPS, ...ARCA_Z_MAPS];

const radToDeg = (rad: number) => Math.round((rad * 180) / Math.PI);
const degToRad = (deg: number) => (deg * Math.PI) / 180;

export default function ArcaTriggerDebug() {
  ...
  const handleExportAll = () => {
    ...
    let code = `// Arca stage configurations - Generated from debug tool\n`;
    code += `// Configured maps: ${configuredMaps.length}\n\n`;
    code += `export const ARCA_CONFIG: Record<string, MapConfig> = {\n`;
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative' }} tabIndex={0} onKeyDown={handleKeyDown}>
      <div style={{ position: 'absolute', top: '10px', left: '10px', zIndex: 1000, background: 'rgba(0,0,0,0.9)', color: 'white', padding: '15px', borderRadius: '8px', fontFamily: 'monospace', fontSize: '13px', width: '280px', maxHeight: 'calc(100vh - 40px)', overflowY: 'auto' }}>
        ...
        <div style={{ marginBottom: '15px' }}>
          ...
          <select value={selectedMap} onChange={(e) => setSelectedMap(e.target.value)} style={{ width: '100%', padding: '8px', background: '#333', color: 'white', border: '1px solid #555', borderRadius: '4px', fontFamily: 'monospace', cursor: 'pointer' }}>
            <optgroup label="Arca A">{ARCA_A_MAPS.map((map) => (<option key={map} value={map}>{allConfigs[map] ? '✓ ' : '  '}{map}</option>))}</optgroup>
            <optgroup label="Arca B">{ARCA_B_MAPS.map((map) => (<option key={map} value={map}>{allConfigs[map] ? '✓ ' : '  '}{map}</option>))}</optgroup>
            <optgroup label="Arca E">{ARCA_E_MAPS.map((map) => (<option key={map} value={map}>{allConfigs[map] ? '✓ ' : '  '}{map}</option>))}</optgroup>
            <optgroup label="Arca Z">{ARCA_Z_MAPS.map((map) => (<option key={map} value={map}>{allConfigs[map] ? '✓ ' : '  '}{map}</option>))}</optgroup>
          </select>
        </div>
      ...
      <a href="/stage/arca" style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 1000, padding: '10px 20px', background: 'rgba(200, 50, 50, 0.9)', color: 'white', textDecoration: 'none', borderRadius: '5px', fontFamily: 'monospace', fontSize: '14px', fontWeight: 'bold' }}>Back to Arca</a>
      <Canvas>
