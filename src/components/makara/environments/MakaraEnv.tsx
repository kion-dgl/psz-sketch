...
// Get makara directory based on mapId prefix (s04a_* -> makara_a, s04b_* -> makara_b, etc.)
function getMakaraDir(mapId: string): string {
  const match = mapId.match(/^s04([a-z])_/);
  if (match) {
    return `stages/makara_${match[1]}`;
  }
  return 'stages/makara_a'; // fallback
}

export function MakaraFloorCollision({ mapId, showVisual = false }: { mapId: string; showVisual?: boolean }) {
  const { setFloorMesh } = useCollision();
  const meshRef = useRef<THREE.Mesh>(null);
  const makaraDir = getMakaraDir(mapId);
  const glbPath = `/${makaraDir}/${mapId}/lndmd/${mapId}_m.glb`;
  ...
  useEffect(() => {
    if (meshRef.current && floorGeometry) {
      const unregister = setFloorMesh(`makara-floor-${mapId}`, meshRef.current);
      return unregister;
    }
...
export default function MakaraEnv({ mapId, showFloorCollision = false }: MakaraEnvProps) {
  const makaraDir = getMakaraDir(mapId);
  const glbPath = `/${makaraDir}/${mapId}/lndmd/${mapId}_m.glb`;
  const { scene } = useGLTF(glbPath);
