...
interface TextureInfo {
  name: string;        // Display name with instance number (e.g., "s04_1_lamp1.png #1")
  key: string;         // Key for settings (e.g., "s04_1_lamp1.png#1")
  filename: string;    // Base filename (e.g., "s04_1_lamp1.png")
  texture: THREE.Texture;
  material: THREE.Material;
}

// Get makara directory based on mapId prefix
function getMakaraDir(mapId: string): string {
  const match = mapId.match(/^s04([a-z])_/);
  if (match) {
    return `stages/makara_${match[1]}`;
  }
  return 'stages/makara_a';
}
...
function MakaraDebugScene({
  ...
  useEffect(() => {
    ...
    const makaraDir = getMakaraDir(selectedMap);
    const glbPath = `/${makaraDir}/${selectedMap}/lndmd/${selectedMap}_m.glb`;
...
const MAKARA_A_MAPS = [
  's04a_ga1', 's04a_ib1', 's04a_ib2', 's04a_ic1', 's04a_ic3',
  's04a_lb1', 's04a_lb3', 's04a_lc1', 's04a_lc2', 's04a_na1',
  's04a_nb2', 's04a_nc2', 's04a_sa1', 's04a_tb3', 's04a_tc3',
  's04a_td1', 's04a_td2', 's04a_xb2'
];
...
const MAKARA_E_MAPS = ['s04e_ia1'];
const MAKARA_Z_MAPS = ['s04z_na1'];

const ALL_MAPS = [...MAKARA_A_MAPS, ...MAKARA_B_MAPS, ...MAKARA_E_MAPS, ...MAKARA_Z_MAPS];

export default function MakaraDebug() {
  const [selectedMap, setSelectedMap] = useState('s04a_ga1');
  ...
  const handleExportJSON = () => {
    ...
    a.href = url;
    a.download = 'makara_texture_config.json';
    a.click();
    ...
    <div style={{ width: '100vw', height: '100vh', display: 'flex' }}>
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '15px' }}>
          ...
          >
            <optgroup label="Makara A">
              {MAKARA_A_MAPS.map((map) => (
                <option key={map} value={map}>{map}</option>
              ))}
            </optgroup>
            <optgroup label="Makara B">
              {MAKARA_B_MAPS.map((map) => (
                <option key={map} value={map}>{map}</option>
              ))}
            </optgroup>
            <optgroup label="Makara E">
              {MAKARA_E_MAPS.map((map) => (
                <option key={map} value={map}>{map}</option>
              ))}
            </optgroup>
            <optgroup label="Makara Z">
              {MAKARA_Z_MAPS.map((map) => (
                <option key={map} value={map}>{map}</option>
              ))}
            </optgroup>
          </select>
